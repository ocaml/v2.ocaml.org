...
module type GRAPH = sig
  ...
  val dfs_fold : t -> node -> ('a -> (int * node) -> 'a) -> 'a -> 'a
end
  ...
  let dfs_fold g c fn acc =
    let node u (t, state) =
      ...
      and dfs_visit t u {d; f; pred; color; acc} =
        ...
        t , {d; f=(Char_map.add u t f); pred; 
             color=Char_map.add u Black color; acc = fn acc (t, u)}
      in
      if Char_map.find u (state.color) = White then dfs_visit t u state
      else (t, state)
    in
    ...
    let initial_state= 
       {d=Char_map.empty;
        ...
        acc=acc}
    in
    (snd (node c (0, initial_state))).acc
...
let l = M.dfs_fold g 'w' (fun acc (_, c) -> acc@[c]) [] 
```
